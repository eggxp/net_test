...
enet_protocol_check_timeouts (ENetHost * host, ENetPeer * peer, ENetEvent * event)
{
    ...
    while (currentCommand != enet_list_end (& peer -> sentReliableCommands))
    {
       ...
       if (check_timeout_count > ENET_PEER_PACKET_THROTTLE_COUNTER && 
           ENET_TIME_DIFFERENCE (host -> serviceTime, peer -> earliestTimeout) >= peer -> timeoutMinimum) {
         // CPU优化. 见文档库:程序文档\三测风险控制\CPU削减.docx
         break;
       }

       //LOG(INFO) << "peer -> connectID:" << peer -> connectID << ".check_timeout_count:" << check_timeout_count;
       //LOG(INFO) << "peer -> timeoutMaximum:" << peer -> timeoutMaximum;
       //LOG(INFO) << "outgoingCommand -> roundTripTimeout:" << outgoingCommand -> roundTripTimeout;
       //LOG(INFO) << "outgoingCommand -> roundTripTimeoutLimit:" << outgoingCommand -> roundTripTimeoutLimit;
       //LOG(INFO) << "ENET_TIME_DIFFERENCE (host -> serviceTime, peer -> earliestTimeout):" << ENET_TIME_DIFFERENCE (host -> serviceTime, peer -> earliestTimeout);
       //LOG(INFO) << "peer -> timeoutMinimum:" << peer -> timeoutMinimum;
       ...
       {
          enet_protocol_notify_disconnect (host, peer, event);
          //LOG(INFO) << "=========================DISCONNECT=====================check_timeout_count:" << check_timeout_count;
          return 1;
       }
       ...
       if (outgoingCommand -> roundTripTimeout > ENET_PEER_DEFAULT_ROUND_TRIP_TIME)
         outgoingCommand -> roundTripTimeout = ENET_PEER_DEFAULT_ROUND_TRIP_TIME;
...
enet_host_service (ENetHost * host, ENetEvent * event, enet_uint32 timeout)
{
    ...
    // double total_times = WallTime_Now();
    if (event != NULL)
    {
        event -> type = ENET_EVENT_TYPE_NONE;
        event -> peer = NULL;
