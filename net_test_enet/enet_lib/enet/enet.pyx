...
cdef extern from "enet/unix.h":
    ctypedef struct ENetBuffer

cdef extern from "enet/enet.h":

    ctypedef enet_uint32 ENetVersion
    ctypedef enet_uint32 (*ENetChecksumCallback)(ENetBuffer* buffers, size_t bufferCount)
    #ctypedef ENetChecksumCallback_t ENetChecksumCallback
    # forward declaration
    ...
    enet_uint32  enet_crc32(ENetBuffer *buff, size_t dataLength)
...
cdef class Peer:
    ...
    def set_timeout(self, timeoutLimit=0, timeoutMinimum=0, timeoutMaximum=0):
        '''
        set_timeout
        '''
        if self.check_valid():
            enet_peer_timeout(self._enet_peer, timeoutLimit, timeoutMinimum, timeoutMaximum)
